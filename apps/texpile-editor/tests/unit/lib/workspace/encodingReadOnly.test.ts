...
/** what a Latin-1 file reads as once decoded leniently */
const LATIN1 = '\\section{Caf\uFFFD}\nDer wei\uFFFD.\n';
const NOT_UTF8 = sourceEncodingError('other');
const UTF16 = sourceEncodingError('utf16le');
...
describe('a file that is not UTF-8', () => {
	...
	it('flags UTF-16 as well as legacy 8-bit', () => {
		const b = buffer();
		b.openTex('C:/p/main.tex', '\\section{Plain}\n', '\n', UTF16);
		expect(b.encodingIssue).toBeTruthy();
		b.onTexInput('edited');
		expect(scheduled).toEqual([]);
	});
